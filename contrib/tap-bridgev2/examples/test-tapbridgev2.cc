...
main(int argc, char* argv[])
{
    ...
    Ptr<TapBridgeV2> tapBridge3 = CreateObject<TapBridgeV2>();
    Ptr<TapBridgeV2> tapBridge4 = CreateObject<TapBridgeV2>();

    tapBridge1->SetImage(img);
    tapBridge2->SetImage(img);
    tapBridge3->SetImage(img);
    tapBridge4->SetImage(img);
    
    tapBridge1->install(devices.Get(0));
    tapBridge2->install(devices.Get(1));
    tapBridge3->install(devices.Get(2));
    tapBridge4->install(devices.Get(3));
